...
# Random Forest
heart_rf_grid <- grid_regular(mtry(range = c(1, 11)),
                              trees(range = c(50, 1000)),
                              min_n(range = c(5, 25)), levels = 10)

# Boosted Trees
heart_bt_grid <- grid_regular(mtry(range = c(1, 11)),
...
From the confusion matrix below, we are able to see how our random forest actually did on our model in terms of prediciton. We see that 
```{r}
...
```{r}
augment(heart_knn_fit, new_data = heartfailure_test) %>%
...
heart_rf_fit %>%
  extract_fit_engine() %>%
  vip(aesthetics = list(fill = "green", color = "pink"))
```

## Result of Top Performing Models 
From 
...
# Random Forest
heart_rf_grid2 <- grid_regular(mtry(range = c(1, 2)),
                              trees(range = c(50, 1000)),
                              min_n(range = c(5, 25)), levels = 10)


# Boosted Trees
heart_bt_grid2 <- grid_regular(mtry(range = c(1, 2)),
...
# Random Forest
show_best(heart_rf_tuned2, metric = "roc_auc")


# Gradient-Boosted Trees
show_best(heart_bt_tuned2, metric = "roc_auc")
...
# Random Forest
heart_rf_train_results2 <- augment(heart_rf_fit2, new_data = heartfailure_train) %>%
  roc_auc(DEATH_EVENT, estimate = .pred_1)



# Boosted Trees
heart_bt_train_resutls2 <- augment(heart_bt_fit2, new_data = heartfailure_train) %>%
  roc_auc(DEATH_EVENT, estimate = .pred_1)



# Random Forest
heart_rf_train_results2


# Boosted Trees
heart_bt_train_resutls2
...
conf_mat(final_rf_model_test2, truth = DEATH_EVENT, .pred_class) %>% autoplot(type = "heatmap")
...
heart_rf_grid3 <- grid_regular(mtry(range = c(1, 3)),
                              trees(range = c(50, 1000)),
                              min_n(range = c(5, 25)), levels = 10)

# Boosted Trees

heart_bt_grid3 <- grid_regular(mtry(range = c(1, 3)),
                              trees(range = c(500, 1000)),
                              learn_rate(range = c(-10, -1)),
                              levels = 10)
...
show_best(heart_rf_tuned3, metric = "roc_auc")

# Gradient-Boosted Trees

show_best(heart_bt_tuned3, metric = "roc_auc")
...
final_heart_rf_model3 <- finalize_workflow(heart_rf_wkflow3, best_heart_rf2)
...
heart_rf_train_results3 <- augment(heart_rf_fit3, new_data = heartfailure_train) %>%
  roc_auc(DEATH_EVENT, estimate = .pred_1)

# Boosted Trees

heart_bt_train_resutls3 <- augment(heart_bt_fit3, new_data = heartfailure_train) %>%
  roc_auc(DEATH_EVENT, estimate = .pred_1)

# Random Forest

heart_rf_train_results3

# Boosted Trees

heart_bt_train_resutls3
...
conf_mat(final_rf_model_test3, truth = DEATH_EVENT, .pred_class) %>% autoplot(type = "heatmap")
```
