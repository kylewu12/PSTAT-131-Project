...
```{r, message = FALSE, fig.cap= "Figure showing variables along with variable type."}
...
```{r, fig.cap="Bar plot showing distribution of patient outcomes with 1 indicating patient died and 0 indicating patient did not die."}
...
```{r, fig.cap= "Correlation plot of continuous variables"}
...
```{r, fig.cap="Box plot showing distribution of patient age by patient outcomes."}
...
```{r, fig.cap="Bar plot showing distribution of patient sex and deaths by sex."}
...
```{r, fig.cap="Box plot showing distribution of creatinine phosphokinase by patient outcomes."}
...
```{r, fig.cap="Box plot showing distribution of ejection fraction by patient outcomes."}
...
```{r, fig.cap="Box plot showing distribution of platelet count by patient outcomes."}
options(scipen = 999)
...
```{r, fig.cap="Box plot showing distribution of serum creatinine by patient outcomes."}
...
```{r, fig.cap="Box plot showing distribution of serum sodium by patient outcomes."}
...
```{r, fig.cap="Graph showing efficacy of various K-Nearest Neighbor models."}
# KNN Autoplot
...
From our Elastic Net Plot, we are able to tune the proportion of lasso penalty and the amount of regularization. From the plot, we are able to see that models with lower lasso penalty proportions tend to have higher AUC-ROC values. We are also able to see that as the amount of regularization increases, we also tend to see models with lower AUC-ROC values. From the different models we tuned on we see that the ideal combiantion for the best mean AUC-ROC value is a penalty if 0.351 and a mixture of 0, meaning 0.351 for the lasso penalty and no regularization.

### Random Forest Plots

```{r, fig.cap="Graphs showing efficacy of various random forest models."}
# Random Forest Autoplot
autoplot(heart_rf_tuned, metric = "roc_auc")
```

For Random Forest models, we are able to tune three hyperparameters: the minimum size of a node through `min_n`, the number of trees through `trees`, and the number of randomly sampled predictors through `mtry`. From the plot, we see that there appears to be a large range of AUC-ROC values that are obtained based on the different numbers. It appears that increasing the minimum node size tends to increase the AUC-ROC values of the models. Having low numbers of randomly selected predictors also appears to increase model performance. From our different models, the random forest with the best performance was the model that randomly sampled 1 predictor, had a model with 155 trees and had a minimum node size of 20.

### Gradient-Boosted Trees Plots

```{r, fig.cap="Graphs displaying efficacy of various gradient-boosted tree models"}
...
In the table below, we summarize the AUC-ROC of each model on the training data. We see that the top two models in our case in terms of AUC-ROC were random forest models and Gradient-Boosted Trees so we will select those two methods as models that we will fit on our testing data and further analyze.
...
Since both of our best models are tree-based models it may be worth explaining in a general way how our models work. As a basis of both our models are decision trees, which essentially identify features that if split on, will create groups that are as different from each other as possible.

Random forests use decision trees as the basis of their model and work when we use a large number of individual decision trees nad improves upon the decision tree algorith [@rfimage]. These trees are each tasked with coming up with a prediction and in the end, the class with the most predictions among the individual trees becomes the model's prediction. The reason why random forest models work is because of the concept that "a large number of relatively uncorrelated models (trees) operating as a committee will outperform any of the individual constituent models" [@randomforest].

<center>
![](randomforestdiagram.png)
</center>

Gradient-boosted trees work by taking weak learners, which to come up wit ha strong learner. In the case of gradient-boosted trees, individual decision trees are the weak learners and they connect to each other sequentially such that each tree tries to minimize the previous trees error. Essentially, each new tree fits the residuals of the previous step which then combine to create a strong learner.

<center>
![](The-architecture-of-Gradient-Boosting-Decision-Tree.png)
</center>

Now that we have chosen random forests and boosted-gradient trees as our two best models, we will fit testing data to them and analyze their results. After fitting the testing data, we are then able to observe the AUC-ROC of the model on testing data. We see that as with the training set, our random forest model performed slightly better than our gradient-boosted trees model with an AUC-ROC value of 0.746 compared to 0.734 respectively. Conversely, we see that the accuracy of gradient-boosted trees (73.8%) is slightly higher than for random forests (70.5%).
...
```{r, fig.cap="Confusion Matrix for full random forest model."}
conf_mat(final_rf_model_test, truth = DEATH_EVENT, .pred_class) %>% autoplot(type = "heatmap")
```

In the figure below, we plot the ROC curve. The more the ROC curve goes up and to the left, the better the model. Although this model does not follow that trajectory fully, its shape and the AUC-ROC value of 0.746 still indicate a model that is relatively efficient at predicting the heart failure outcome of patients.

```{r, fig.cap="ROC curve for full random forest model."}
...
The confusion matrix below breaks down the predictions made by the Gradient-Boosted Trees model. Again, it is important to note that accuracy may be significantly affected purely because of the low number of observations in the training set, but this model exhibited 73.8% accuracy on the testing data.


```{r, fig.cap="Confusion Matrix for full gradient-boosted trees model."}
...
```{r, fig.cap="ROC curve for full gradient-boosted trees model."}
...
When creating a random forest model and gradient-boosted tree models, we are able to create variable importance plots, which allows us to visualize which variables were most effective in improving the model. We are able to see that variables which were dummy coded (high blood pressure, diabetes, smoking, and sex) appear to have relatively small effects in predicting the outcome of heart failure patients. We also see the ranking of the importance of the variables in the plot with the variable most useful in improving the model at the top. 

```{r, fig.cap="Variable Importance Plot for Random Forest Algorithm (Top) and Gradient-Boosted Trees (Bottom) using Gini Impurity"}
...
```{r, fig.cap= "Confusion Matrix for Random Forest (Left) and Gradient-Boosted Trees algorithm (Right) for models using top two variables (Serum Creatinine and Ejection Fraction)"}
...
From the confusion matrices above, we see that both the random forest and boosted-gradient tree algorithms performed equally well on the testing set. However, it is again important to emphasize that according to the AUC-ROC scores, the random forest is a slightly better model and we can further see this from the ROC curves displayed below.

```{r, fig.cap="ROC curves of Random Forest (left) and Gradient-Boosted Trees algorithm (right) for models using top two variables (Serum Creatinine and Ejection Fraction)"}
...
## Conclusion
In this study, we attempted to predict the chance that a patient would die from heart failure based off of patient data. We tried out seven different models including logistic regression, K-nearest neighbors (KNN), linear discriminant analysis (LDA), quadratic discriminant analysis (QDA),  elastic net, random forests, and gradient-boosted trees. In the end, we found that random forests and gradient-boosted trees were the top two performing models and we found that when we used all predictors, they performed relatively well with AUC-ROC scores of 0.746 and 0.734 respectively.

After visualizing each respective variable importance plot, it became apparent that some variables, especially variables that were dummy-coded appeared to have very little importance in both random forest and gradient-boosted tree models so we elected to see if creating simpler models allowed for a more efficient model that was able to just as well. Interestingly, we found that some of our simpler models actually outperformed the full model, possibly because our full model had overfit the data and was measuring noise rather than underlying patterns. Our best model only required the use of our two most important variables serum creatinine and ejection fraction and presented AUC-ROC scores of 0.855 compared to 0.746 for random forest models and 0.826 compared to 0.734 for gradient-boosted trees models. Both models also presented 77% accuracy in predicting the outcome of patients.

Following this analysis, there are many possible paths that we may be able to continue on, the most obvious one being gathering data from more patients. The data set was relatively small at 299 total observations and increasing the number of observations would perhaps allow our models to pick up even more information about heart failure. Additionally, our data was taken from a group of patients in Faisalabad, Pakistan, which means that patients may have shared other characteristics not accounted for in data so collecting patient data from other areas may also allow us to get a better understanding of heart failure deaths. We could also perhaps understand more about patients by asking them other questions such as whether they had any comorbidities at the time of patient data collection.
...
Nevertheless, this study provides information that could be extremely useful for medical professionals in predicting the health outcomes of patients. We were able to gather that by using serum creatinine and ejection fraction, we could create models that possessed high efficacy in predicting the outcome of heart failure patients. Our models indicate that if a patient were at high risk of heart failure, medical professionals could simply run a blood test for serum creatinine and an echocardiogram for ejection fraction to help them understand the seriousness of patient condition, which could direct further actions. Overall, this means that the diagnostic process could be made not only more efficient, but more cost effective as well.
